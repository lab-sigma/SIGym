import numpy as np
from General_Stackelberg.dynamic_stackelberg import SSE
...
class Follower:
    ...
    def DeltaSuboptimalResponse(self, x, R, delta=0.1):
        ...
        for ind, j in enumerate(delta_responses):
            temp = 0.0
            for i in range(m):
                temp += x[i]*R[i][j]
            delta_response_leader_utilities[ind] = temp
        return_j = np.argmin(delta_response_leader_utilities)
        return delta_responses[return_j]
    
    
class Platform:
    def __init__(self, m, n, behavior_mode) -> None:
        self.R = np.random.rand(m, n)
        self.C = np.random.rand(m, n)
        self.num_types = 1
        self.m, self.n = len(self.R), len(self.R[0])
        self.pi = [1.0/self.num_types for _ in range(self.num_types)]
        self.cum_u = 0.0
        self.follower = Follower(self.C, behavior_mode)
    
    def compute_utility(self, i_t, j_t):
        return self.R[i_t][j_t]
    ...
    def step(self, x, follower):
        ...
        j_t = self.follower.response(x, R)
        return i_t, j_t