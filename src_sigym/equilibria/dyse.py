...
"""
    Solves for the dySE for the repeated bayesian stackelberg game given by R, C, pi, and T
        ...
        T:  number of rounds
"""
def dySE_FAND(R, C, pi, T, security=True):
    env = gp.Env(empty=True)
    env.setParam('OutputFlag', 0)
    env.start()
    M = gp.Model("dySE", env=env)
        ...
        for t in range(T):
            ...
            for history in range(nv):
                ...
                if t > 0:
                    ...
                    for st in range(t):
                        jst = subhistory % n
                        y_thetas.append(y[theta][st][jst])
                        subhistory = subhistory // n
                    for yt in y_thetas:
                        M.addConstr(zt_theta_hist <= yt)
                    M.addConstr(zt_theta_hist >= gp.quicksum(y_thetas) - t, name="geqand_{},{},{}".format(theta, t, get_history_str(history, n, t+1), history))
                else:
                    M.addConstr(zt_theta_hist == y[theta][t][history], name="eq_{},{},{},{}".format(theta, t, get_history_str(history, n, t+1), history))
                zt_theta.append(zt_theta_hist)
